...
function change1(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(address1.getElementsByTagName('select')[i].selectedIndex-1));
	};
	var ss=address1.getElementsByTagName('select')[v];
	with(ss){
		length = 0;
		options[0]=new Option(opt0[v],opt0[v]);
		if(v && address1.getElementsByTagName('select')[v-1].selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
		...
		if(++v<s.length){change1(v);}
	}//End with
}
function change2(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(address2.getElementsByTagName('select')[i].selectedIndex-1));
	};
	var ss=address2.getElementsByTagName('select')[v];
	with(ss){
		length = 0;
		options[0]=new Option(opt0[v],opt0[v]);
		if(v && address2.getElementsByTagName('select')[v-1].selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
...
function change3(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(address3.getElementsByTagName('select')[i].selectedIndex-1));
	};
	var ss=address3.getElementsByTagName('select')[v];
	with(ss){
		length = 0;
		options[0]=new Option(opt0[v],opt0[v]);
		if(v && address3.getElementsByTagName('select')[v-1].selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);
